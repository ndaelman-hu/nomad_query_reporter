...
if __name__ == "__main__":
    ...
    # Read IDs
    ids, id_type = [], ""
    if len(args.upload_id) > 0:
        for upload_id in args.upload_id:
            if not check_upload_id(upload_id):
                print(f"Invalid upload ID: {upload_id}")
                sys.exit(1)
        ids.extend(args.upload_id)
        id_type = "upload_id"
    elif len(args.entry_id) > 0:
        ids.extend(args.entry_id)
        id_type = "entry_id"
    else:
    ...
    nomad_query = substitute_tags(
        nomad_complete_prompt(f"{nq_dir}/{args.nomad_query_type}.json"),
        {id_type: ids},
    )
